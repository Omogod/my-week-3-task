...
async function driverReport() {
  ...
  for (let elem of newReport) {
    ...
    if (totalEarnings[elem.driverID]) {
      totalEarnings[elem.driverID] += parseFloat(
        String(elem.billedAmount).split(",").join("")
      );
    } else {
      totalEarnings[elem.driverID] = parseFloat(
        String(elem.billedAmount).split(",").join("")
    ...
    if (elem.isCash) {
      if (cashAmount[elem.driverID]) {
        cashAmount[elem.driverID] += parseFloat(
          String(elem["billedAmount"]).split(",").join("")
        );
      } else {
        cashAmount[elem.driverID] = parseFloat(
    ...
    if (!elem.isCash) {
      if (nonCashAmount[elem.driverID]) {
        nonCashAmount[elem.driverID] += parseFloat(
          String(elem["billedAmount"]).split(",").join("")
        );
      } else {
        nonCashAmount[elem.driverID] = parseFloat(
  ...
  for (let elem in correctDriverInfo) {
    ...
    if (correctDriverInfo[elem]) {
      ...
      driverInfo["noOfVehicles"] =
        correctDriverInfo[elem].value.vehicleID.length;
      driverInfo["vehicles"] = [];
